...
namespace DropRateStudio.TechTest.GridGeneration
{
    public sealed class IsometricGridGenerator : GenericSingleton<IsometricGridGenerator>
    {
        ...
        /// </summary>
        /// <param name="isFirstGridGeneration">True : First grid generation, False : A grid already exist.</param>
        private void ManageGeneration(bool isFirstGridGeneration)
        {
            // Check if a generetion isn't currently running before run a new one (avoid error and perf issue)
            _generationCoroutine ??= StartCoroutine(GenerateGridCheck(isFirstGridGeneration));
        }
        ...
        private IEnumerator ClearGrid()
        {
            ...
            Extensions.DestroyAllChildren(_tilesParentTransform);
            Extensions.DestroyAllChildren(_obstaclesParentTransform);
